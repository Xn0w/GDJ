...
SELECT
      ...
      , FLOOR(EXTRACT(SECOND FROM SYSTIMESTAMP)) AS 시
  FROM
        DUAL;
        
-- 4. 단위(년,월,일,시,분,초) 추출은 TO_CHAR 함수로도 가능함
SELECT
...
SELECT
      ...
      , TO_CHAR(SYSDATE + 0.5, 'MM/DD AM HH24:MI:SS') AS "12시간 후"
      , TO_CHAR(SYSDATE + (1 / 24), 'MM/DD AM HH24:MI:SS') AS "1시간 후"
  FROM
        DUAL;
...
-- 6. 경과한 개월 수
--    MONTH_BETWEEN(최근날짜, 이전날짜) : 두 날짜 사이의 경과한 개월 수
SELECT
