...
BEGIN
    ...
    ELSE
        GRADE := 'F';
    END IF;
    DBMS_OUTPUT.PUT_LINE(SCORE || '점은 ' || GRADE || '입니다.');
END;    
...
BEGIN
    ...
    ELSE
        RESULT := '저연봉';
    END IF;
    DBMS_OUTPUT.PUT_LINE('연봉 ' || V_SALARY || '원은 ' || RESULT || '입니다.');
END;    
...
-- DEPARTMENT_ID가 50인 사원을 제외하고 연봉합계
DECLARE
    EMP_NO EMPLOYEES.EMPLOYEE_ID%TYPE;
    SAL EMPLOYEES.SALARY%TYPE;
...
BEGIN
    SELECT FIRST_NAME
      ...
      FROM EMPLOYEES
    -- WHERE EMPLOYEE = 0;
     WHERE EMPLOYEE_ID = 50;   -- 존재하지 않는 사원 조회
EXCEPTION
