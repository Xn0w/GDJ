/*
1.
테이블 이름(TABLE_NAME)이 'EMP'인 테이블에 존재하는 인덱스의 이름(INDEX_NAME)과 
해당 인덱스가 설정된 칼럼 이름(COLUMN_NAME)을 조회할 수 있는 쿼리문을 작성하시오.
...
/*
2.
현재 접속한 사용자(USER)가 작성한 모든 VIEW의 목록을 조회할 수 있는 쿼리문을 작성하시오.
...
CREATE TABLE PRODUCT(
    PRODUCT_NAME VARCHAR(20 BYTE)
);
CREATE INDEX IDN_NAME ON PRODUCT(PRODUCT_NAME);

/*
4.
PRODUCT 테이블의 모든 칼럼을 복사하여 NEW_PRODUCT이라는
이름의 새 테이블을 생성하는 쿼리문을 작성하시오.
...
/*
5.
다음 지시사항의 대상 테이블과 대상 칼럼 정보를 이용해서
새로운 뷰(VIEW)를 생성하는 쿼리문을 작성하시오. (10점)
    1) 대상 테이블 : PRODUCT
...
/*
7.
현재 사용자 계정(USER)에 작성된 각종 제약조건을 확인할 수 있는
테이블의 구조를 확인하는 쿼리문을 작성하시오. (5점)
*/
    SELECT *
      FROM COLS;
...
/*
10.
어떤 테이블에 삽입, 수정, 삭제된 행(ROW) 정보를 실제로 DB에 반영하기 위해서
사용하는 쿼리문을 작성하시오. (5점)
*/
COMMIT
