...
-- 3. 그룹화 함수 사용
-- 집계함수(그룹함수)는 GROUP BY절에 해당 칼럼이 없어도 SELECT절에서 조회할 수 있다
SELECT
       DEPARTMENT_ID
...
-- 1) 부서번호가 100 미만인 부서들의 연봉평균을 조회하시오(WHERE절, HAVING절 모두 가능한 조건)
...
-- 4. 부서명(DEPARTMENT_NAME)의 첫 2글자로 그룹화하여 해당 그룹의 개수 조회하기 -- 개수 조회 = COUNT(*)
