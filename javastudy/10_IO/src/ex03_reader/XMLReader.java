	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int i = 0; i < nodeList.getLength(); i++) {
				Node node = nodeList.item(i);	// 줄바꿈과(#text)과 <product>태그로 구성
				if(node.getNodeType() == Node.ELEMENT_NODE) {	// 노드가 Element인가?(줄바꿈 #text 제외되고 <product> 태그만 남음)
					...
					for(int j = 0; j <nodeList2.getLength(); j++) {
						...
						if(node2.getNodeType() == Node.ELEMENT_NODE) {	// 노드가 Element인가?(줄바꿈 #text 제외되고 <product> 태그만 남음)
							switch(node2.getNodeName()) {
								case "number" : product.setNumber(node2.getTextContent());
								break;
								case "name" : product.setName(node2.getTextContent());
								break;
